...
用法：
    python scripts/participants/seed_participants.py [--dry-run] [--backfill-replays]

- 账号/模型 upsert 按稳定 id，幂等；re-run 无副作用。
- ``--backfill-replays``：读 ``artifacts/replays/*/meta.json``（player_names +
...
def _resolve_registry_paths() -> list[Path]:
    paths: list[Path] = []
    in_repo = _REPO_ROOT / "configs" / "ladder" / "seasons"
    if in_repo.exists():
...
def _replays_dir() -> Path:
    """回放目录：默认 artifacts/replays，可用 KEQING_PARTICIPANT_REPLAYS_DIR 覆盖（测试用）。"""
    env = os.environ.get("KEQING_PARTICIPANT_REPLAYS_DIR")
    if env:
        return Path(env)
    return _REPO_ROOT / "artifacts" / "replays"
...
def main() -> int:
    ...
    parser.add_argument("--backfill-replays", action="store_true", help="回填 artifacts/replays 到统一账本")
