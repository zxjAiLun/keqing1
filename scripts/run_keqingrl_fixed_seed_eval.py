...
def _summary_markdown(args: argparse.Namespace, report) -> str:
    lines = [
        ...
        f"- learner_win_rate: `{report.learner_win_rate:.6g}`",
        f"- learner_deal_in_rate: `{report.learner_deal_in_rate:.6g}`",
        f"- learner_call_rate: `{report.learner_call_rate:.6g}`",
        f"- learner_riichi_rate: `{report.learner_riichi_rate:.6g}`",
        f"- illegal_action_rate_fail_closed: `{report.illegal_action_rate_fail_closed:.6g}`",
        f"- fallback_rate_fail_closed: `{report.fallback_rate_fail_closed:.6g}`",
        f"- forced_terminal_missed_fail_closed: `{report.forced_terminal_missed_fail_closed}`",
        "- fail_closed_note: `hard raise gates; not observed recoverable event rates`",
