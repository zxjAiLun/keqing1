...
$ExpDir = "artifacts\experiments\model_pool_2026_07\V3_final_rank_mc_warmstart_2026_07"
...
Invoke-Logged "00_audit_data" @(
    "uv", "run", "--no-sync", "python", "scripts/mortal/audit_population_synthetic_dataset.py"
)
Invoke-Logged "01_reward_preflight" @(
    "uv", "run", "--no-sync", "python", "scripts/mortal/preflight_final_rank_mc.py",
    "--output", "$ExpDir\reward_preflight.json"
)
Invoke-Logged "02_prepare_v3" @(
    "uv", "run", "--no-sync", "python", "scripts/mortal/prepare_v3_final_rank_mc_warmstart.py"
)
...
if ($RunTraining) {
    ...
    $Train = @(
        ...
        "--device", "cuda", "--num-workers", "0", "--seed", "20260716", "--data-seed", "20260716",
    ...
    Invoke-Logged "03_train_v3_74000" $Train
}
