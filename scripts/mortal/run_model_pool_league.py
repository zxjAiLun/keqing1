...
# Allow direct execution via ``python scripts/mortal/run_model_pool_league.py``.
...
def aggregate_family_stats(report: dict) -> dict[str, dict]:
    raw_totals: dict[str, defaultdict[str, float]] = defaultdict(lambda: defaultdict(float))
    for account in report["accounts"]:
        family = str(account["model_label"])
        raw = report["stat_report"]["players"][account["account_id"]]["raw"]
...
def write_summary(output_dir: Path, stats: dict[str, dict], report: dict) -> None:
    document = {
        "schema": "keqing.mortal.model_pool_league.v1",
        "lineups": [name for name, _ in LINEUPS],
        "games_per_lineup": report["games"] // len(LINEUPS),
        "games_total": report["games"],
        "rank_points": RANK_PTS,
        "models": stats,
        "platform_accounts_dir": str(output_dir / "platform_accounts"),
    }
...
def main() -> None:
    ...
    for index, (lineup_name, labels) in enumerate(LINEUPS):
        ...
        command = [
            sys.executable,
            str(Path("scripts/mortal/four_player_native.py").resolve()),
    ...
    platform_report = build_report(
        log_dirs=[block_dir / "logs" for block_dir in block_dirs],
        output_dir=output_dir / "platform_accounts",
        mortal_root=Path("third_party/Mortal"),
        platform_model_label=None,
        rank_points=tuple(RANK_PTS),
        preserve_log_dir_order=True,
    )
    stats = aggregate_family_stats(platform_report)
    write_summary(output_dir, stats, platform_report)
    manifest = {
