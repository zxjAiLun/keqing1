...
$status = @(git -c core.excludesFile=NUL status --porcelain 2>$null)
...
Invoke-Logged "00_prepare_reward_ab" @(
    "scripts/mortal/prepare_reward_ab.py",
    "--experiment-id", $ExpId,
    "--seeds", ($Seeds -join ","),
...
if (-not (Test-Path -LiteralPath $Preflight)) {
    Invoke-Logged "01_reward_preflight" @(
        "scripts/mortal/preflight_reward_distribution.py",
...
Invoke-Logged "02_audit_reward_ab" @(
    "scripts/mortal/audit_reward_ab.py",
    "--root", $ExpDir,
    "--seeds", ($Seeds -join ","),
    "--output", $AuditPath
)
