...
当前全局优先级保持为：
1. `xmodel1` 的模型架构与训练窗口稳定性
2. `xmodel1` 的训练流程与评测质量
3. `keqingv4` 的 Rust ownership 收口
4. 共享 Rust semantic core 巩固
5. runtime / replay / bot 兼容面清理

这意味着项目当前不再优先做旧模型族的补丁式修修补补，也不再把 battle/replay 侧问题当成主目标，除非它们直接阻塞模型主线。
...
现在真正缺的不是新结构，而是用户执行出来的证据：
- full preprocess
- smoke train
- slice/review acceptance
...
因此 `keqingv4` 现在的重点是验证并冻结当前 backup snapshot，而不是继续加新结构。
...
最关键的验收序列仍然是：
1. 对 `ds1 + ds2 + ds3` 跑 `xmodel1` full preprocess gate
2. gate 通过后跑 smoke train
3. 对 `reach / call / none / hora` 等 slice 做 review acceptance
4. 并行完成 `keqingv4` 的 focused verification
5. 把共享 Rust fail-closed 边界冻结为下一轮基线

在这些证据没有跑出来之前，代码侧“已经准备好”不等于模型质量已经被证明。
