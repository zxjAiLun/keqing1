...
The audit implementation is [audit_replay_distribution.py](../../scripts/mortal/audit_replay_distribution.py). It supports CUDA preflight, bounded file batches, progress output, exact duplicate hashing, and frozen-parent Q support. The full run used CUDA, `file_batch_size=5`, `q_batch_size=512`, and `require_cuda`.
