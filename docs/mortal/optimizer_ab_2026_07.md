...
Do not promote preserved Adam as the project-wide optimizer default solely from this result. The result is a reproducible positive direction across the three matched seeds, but both uncertainty intervals include zero and the evaluation used only three training seeds.
...
4. If optimizer work continues, add new matched training seeds or repeat the A/B with a pre-registered minimum effect and the same 1000-hanchan protocol. Otherwise move back to the larger data/lineage question rather than making a serving-model promotion.
