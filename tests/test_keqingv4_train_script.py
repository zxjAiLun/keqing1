...
def _write_legacy_keqingv4_checkpoint(path: Path) -> None:
    model = KeqingV4Model(
        hidden_dim=64,
        num_res_blocks=2,
        action_embed_dim=16,
...
def test_train_keqingv4_script_rejects_resume_without_checkpoint_metadata(tmp_path: Path):
    data_root = tmp_path / "processed_v4"
    ds1 = data_root / "ds1"
    ds2 = data_root / "ds2"
    ds1.mkdir(parents=True)
    ...
    _write_keqingv4_contract_npz(ds1 / "a.npz")
    _write_keqingv4_contract_npz(ds2 / "b.npz")
    resume_path = tmp_path / "legacy_keqingv4.pth"
    _write_legacy_keqingv4_checkpoint(resume_path)
    ...
    assert result.returncode != 0
    combined = result.stdout + result.stderr
    assert "missing required keqingv4 checkpoint metadata" in combined
