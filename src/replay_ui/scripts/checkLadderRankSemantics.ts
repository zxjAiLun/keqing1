// src/replay_ui/scripts/checkLadderRankSemantics.ts
...
// 3. scoring 类型包含 system / version / room_policy；
// 4. 天梯表展示段位名与 PT 升段进度；
// 5. 模型聚合展示最高/中位段位与段位分布，不再无条件平均 PT；
// 6. 账号页展示段位变化（transition）与卓别；
// 7. report builder 不再用固定七段常量计算 ledger；
// 8. 新测试文件在 pytest python_files 白名单；
// 9. manifest 写入 scoring 契约（scoring_config_hash）。
...
check(/room_policy\?: string/.test(types), 'LadderSeasonScoring 应包含 room_policy');
...
check(/table_room/.test(accountPage), '账号页应展示卓别（table_room）');
check(/promotions/.test(accountPage), '账号页应展示升段次数');
check(/demotions/.test(accountPage), '账号页应展示降段次数');

// 7. report builder 不再用固定七段常量计算 ledger
const builder = readFileSync(resolve(REPO, 'scripts/mortal/build_platform_account_report.py'), 'utf8');
...
// 9. manifest 写入 scoring 契约
const publisher = readFileSync(resolve(REPO, 'scripts/mortal/publish_ladder_snapshot.py'), 'utf8');
...
console.log('ladder rank semantics OK (rank sort, rank types, model aggregation, transitions, builder engine)');
