"""缓存数据集：直接加载预处理好的 .npz 文件，跳过实时解析和花色增强。

设计：
- IterableDataset，流式加载 .npz 文件
- 维护 shuffle_buffer（大小 2000），随机 pop 样本
- 多 epoch 自动重 shuffle 文件列表
- .npz 格式：tile_feat(float16), scalar(float16), mask(uint8), action_idx(int16), value(float16)
"""
...
def _apply_suit_perm(tile_feat: np.ndarray, mask: np.ndarray, action_idx: int, perm: tuple) -> Tuple:
    """在特征层面做花色置换。tile_feat: (128,34), mask: (45,), action_idx: int。"""
    # tile_feat axis=1 的 0-26 做 swap
    ...
    # action_idx: dahai(0-33) 需要对应置换，其他动作不变
    new_action = action_idx
    if action_idx < 27:  # dahai 且是数牌
        suit_i = action_idx // 9
        rank = action_idx % 9
        new_suit_i = perm.index(suit_i)
        new_action = new_suit_i * 9 + rank

    # mask: dahai 部分(0-26) 同样置换
    new_mask = mask.copy()
