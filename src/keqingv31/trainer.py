...
def _make_v31_task(cfg: Dict) -> TaskSpec:
    ...
    offense_loss_weight = float(cfg.get("offense_loss_weight", 0.35))
    defense_loss_weight = float(cfg.get("defense_loss_weight", 0.35))
    gate_reg_weight = float(cfg.get("gate_reg_weight", 0.01))
    ...
    def compute_extra_loss(model, device: torch.device, batch_data: Dict, is_train: bool, batch_idx: int):
        ...
        offense_pred = aux["offense_value"].squeeze(-1)
        defense_pred = aux["defense_risk"].squeeze(-1)
        gate = aux["stream_gate"]
        ...
        offense_target = batch_data["score_delta_target"] + 0.5 * batch_data["win_target"]
        defense_target = batch_data["dealin_target"]
        offense_loss = F.smooth_l1_loss(offense_pred, offense_target)
        defense_loss = F.binary_cross_entropy_with_logits(defense_pred, defense_target)
        gate_entropy = -(gate.clamp_min(1e-8) * gate.clamp_min(1e-8).log()).sum(dim=-1).mean()
        gate_reg = -gate_entropy

        loss = (
            score_loss_weight * score_loss
            + win_loss_weight * win_loss
            + dealin_loss_weight * dealin_loss
            + offense_loss_weight * offense_loss
            + defense_loss_weight * defense_loss
        ...
        return loss, {
            ...
            "offense_loss": float(offense_loss.item()),
            "defense_loss": float(defense_loss.item()),
            "gate_entropy": float(gate_entropy.item()),
            "score_pred_mean": float(score_pred.mean().item()),
            "offense_pred_mean": float(offense_pred.mean().item()),
            "defense_pred_mean": float(torch.sigmoid(defense_pred).mean().item()),
